...
################### CONFIGURATION PART: TO SPECIFY THE NETWORK ##########################
# HOSTS CONFIGURATION (for PCs etc with one interface, i.e. one straigt-through cable connected)
NHosts=5
# Settings of network (non-management) IP address of node at eth1:
...
# Here: "hostcable1" refers to the name of the internal network used by adapters in VirtualBox
...
############################ FIXED PART; BELOW THIS LINE #################################

# TODO: Need a way to assign default gateways to hosts. Options:
# a) Configure it manually above
# b) Assume that all routers are given the suffic .1 at the network X.Y.Z.0
# c) PREFERRED: Automatically detect all network prefixes of the routers, and assign a default gateway
#    to a host according to prefix matching.
...
# REQUIRES:
#
# - A Host-template node with one internal adapter enabled (eth0) for internal management communication; 
#   with managment IP address set to 192.168.100.200.
#   It will be cloned into a number of hosts, each given managment IP addresses on eth0 192.168.100.201, .202, .203 ... etc
#
# - A Switch-template node with one internal adapter enabled (eth0) for internal management communication; 
#   with IP address set to 192.168.100.140
#   It will be cloned into a number of hosts, each given managment IP addresses on eth0 192.168.100.141, .142, .143 ... etc
#
# - A Router-template node with one internal adapter enabled (eth0) for internal management communication; 
#   with IP address set to 192.168.100.170
#   It will be cloned into a number of hosts, each given managment IP addresses on eth0 192.168.100.171, .172, .173 ... etc
#
# All template nodes requires ssh keys installed
...
if [ "$1" = "clone" ]
  then
    ...
    do
       ...
       # Changing away from 192.168.100.200 into a new management IP address in bootlocal.sh (text replacement with sed):
       # We are storing the new bootlocal.sh file in a template file (bootlocal.sh_template stored at the VM), 
       # that we manipulate and copy into the new bootlocal.sh:
       ...
       # Setting network (non-management) IP address of node at eth1, according to settings in CONFIGURATION part above:
